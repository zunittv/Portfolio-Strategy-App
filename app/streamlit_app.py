...
# --- dynamic import fallback if portfolio_engine isn't on sys.path ---
try:
    import portfolio_engine  # noqa: F401
except Exception:
    import importlib.util, importlib.machinery, types, os, sys
    _mod_name = "portfolio_engine"
    _file = os.path.join(HERE, "portfolio_engine.py")
    if os.path.exists(_file):
        loader = importlib.machinery.SourceFileLoader(_mod_name, _file)
        spec = importlib.util.spec_from_loader(_mod_name, loader)
        module = importlib.util.module_from_spec(spec)
        loader.exec_module(module)
        sys.modules[_mod_name] = module
    else:
        raise FileNotFoundError(f"portfolio_engine.py not found at {_file}")
